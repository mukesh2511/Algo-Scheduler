...
export default function SJFScheduler() {
  ...
  function tick() {
    ...
    // 2) SRTF selection/preemption at every tick
    // Ensure all items in queue have 'remaining'
    if (queueRef.current.length > 0) {
      queueRef.current = queueRef.current.map((q) =>
        q.remaining == null ? { ...q, remaining: q.burst } : q
      );
    }

    // Build candidate set: running (if any) + queue
    const candidates = runningRef.current
      ? [runningRef.current, ...queueRef.current]
      : [...queueRef.current];

    if (candidates.length > 0) {
      // Pick the process with the smallest remaining time (tie-breaker: arrival, then id)
      const selected = [...candidates].sort(
        (a, b) =>
          a.remaining - b.remaining || a.arrival - b.arrival || String(a.id).localeCompare(String(b.id))
      )[0];

      const isSameAsRunning =
        runningRef.current && selected.id === runningRef.current.id;

      if (!isSameAsRunning) {
        // Preempt: move current running back to queue
        if (runningRef.current) {
          queueRef.current.push(runningRef.current);
        }

        // Remove selected from queue (if it came from queue)
        const selIdx = queueRef.current.findIndex((x) => x.id === selected.id);
        if (selIdx !== -1) queueRef.current.splice(selIdx, 1);

        // Dispatch selected
        runningRef.current = { ...selected, startTime: t, remaining: selected.remaining };
        runningId.current = selected.id;
        setRunning({ ...runningRef.current });
        setReadyQueue([...queueRef.current]);
        setLog((l) => [...l, `t=${t}: ${selected.id} dispatched to CPU (SRTF)`]);
      }
    } else {
      // Nothing to run
      if (!runningRef.current) idleRef.current = true;
    }
    ...
    if (runningRef.current) {
      ...
      // No running process, record idle gap if necessary (optional: show as "IDLE")
      // For a clean Gantt, we won't add explicit idle segments; the space will be blank.
    }
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="text-2xl font-bold">
        Sortest Remaining Time First (SRTF) Scheduling Simulator
      </h1>
      <p className="text-sm text-gray-600 mt-1">
        Enter processes, then simulate SRTF with a live animation.
      </p>

...
export default function SJFScheduler() {
  ...
  function tick() {
    ...
    if (newArrivals.length) {
      queueRef.current = [...queueRef.current, ...newArrivals];
    ...
    // // 2) If CPU idle, pull next from ready queue (FCFS)
    // if (!runningRef.current) {
    //   const next = queueRef.current.shift() || null;
    //   if (next) {
    //     runningRef.current = { ...next, remaining: next.burst, startTime: t };
    //     runningId.current = next.id;
    //     setRunning({ ...runningRef.current });
    //     setReadyQueue([...queueRef.current]);
    //     setLog((l) => [...l, `t=${t}: ${next.id} dispatched to CPU`]);
    //   } else {
    //     // CPU idle this tick
    //     idleRef.current = true;
    //   }
    // }
    // 2) If CPU idle, pick the job with the smallest burst time (SJF)
    if (!runningRef.current) {
      if (queueRef.current.length > 0) {
        // sort ready queue by burst time
        queueRef.current.sort((a, b) => a.burst - b.burst);

        // pick the shortest burst
        const next = queueRef.current.shift();
        runningRef.current = { ...next, remaining: next.burst, startTime: t };
        runningId.current = next.id;
        setRunning({ ...runningRef.current });
        setReadyQueue([...queueRef.current]);
        setLog((l) => [...l, `t=${t}: ${next.id} dispatched to CPU (SJF)`]);
      } else {
        idleRef.current = true;
      }
    }
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="text-2xl font-bold">
        Sortest Job First (SJF) Scheduling Simulator
      </h1>
      <p className="text-sm text-gray-600 mt-1">
        Enter processes, then simulate SJF with a live animation.
      </p>
